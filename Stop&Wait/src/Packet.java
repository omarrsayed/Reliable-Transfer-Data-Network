...
public class Packet {
	...
	public void decode(byte[] data) {
		...
		length = ByteBuffer.wrap(data, 0, 4).getInt();
		if(length == 0) // In case of termination packet
			return;
		checksum = ByteBuffer.wrap(data, 4, 4).getInt();
		seqno = ByteBuffer.wrap(data, 8, 4).getInt();
		dataBytes = new byte[length - 12];
		dataBytes = Arrays.copyOfRange(data, 12, length);
