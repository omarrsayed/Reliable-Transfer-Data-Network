...
public class Client {
	...
	ArrayList<Packet> packetsBuffer;
	...
	public Client(String clientinfo) throws IOException {
		...
		packetsBuffer = new ArrayList<Packet>();
	...
	public Packet retreivePacket(int sequenceNumber) {
		
		for(int i = 0; i < packetsBuffer.size(); i++) {
			if(packetsBuffer.get(i).getSeqno() == sequenceNumber)
				return packetsBuffer.remove(i);
		}
		return null;
	...
	public int[] reallocateBuffer(int[] buffer, int maxSequenceNumber) {
		
		return Arrays.copyOf(buffer, maxSequenceNumber + 1);
